...
class QuietDogEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, xml_path="go1_with_shoes.xml"):
        ...
        self.per_joint_delta = np.array([0.06, 0.12, 0.08] * 4, dtype=np.float32)
        ...
        forb_geom_ids = []
        for bname in forbidden_bodies:
            bid = self.model.body(bname).id
            adr = int(self.model.body_geomadr[bid])
            num = int(self.model.body_geomnum[bid])
            if num > 0:
                forb_geom_ids.extend(range(adr, adr + num))
        
        forb_geom_ids = [g for g in forb_geom_ids if g not in self.silicone_geom_ids]
        self.forbidden_geom_ids = set(forb_geom_ids)
        ...
        self._forbidden_contact_streak = 0
        self._forbidden_contact_limit = 8 
    ...
    def _compute_reward(self) -> float:
        ...
        for i in range(self.data.ncon):
            ...
            if ((g1 in self.forbidden_geom_ids and g2 == self.floor_geom_id) or
                (g2 in self.forbidden_geom_ids and g1 == self.floor_geom_id)):
                forbidden_count += 1.0
                mujoco.mj_contactForce(self.model, self.data, i, fbuf)
                forbidden_force += max(0.0, float(fbuf[2]))

        shoe_contact_reward = 0.04 * silicone_contact_count + 0.00008 * silicone_normal_force
        forbidden_contact_pen = 1.2 * forbidden_count + 0.003 * forbidden_force  # STRONG

        # forwrrd velocity tracking
        track = np.exp(-0.5 * ((vx - self.v_cmd) / self.v_sigma) ** 2)
        track_reward = 1.5 * float(track)

        # try 2 contacts (trot-ish) over 4 (standing)
    ...
    def step(self, action):
        ...
        # forbidden-contact >:( for early termination
        forbidden_touch = False
        for i in range(self.data.ncon):
            c = self.data.contact[i]
            g1, g2 = int(c.geom1), int(c.geom2)
            if ((g1 in self.forbidden_geom_ids and g2 == self.floor_geom_id) or
                ...
                break
        if forbidden_touch:
            self._forbidden_contact_streak += 1
        else:
            self._forbidden_contact_streak = 0
        ...
        terminated = (
            ...
            or (self._forbidden_contact_streak >= self._forbidden_contact_limit)
        )
        truncated = self.current_step >= self.max_episode_steps
        info = {}
